...
def connect_to_sheet(sheet_name):
    ...
    try:
        # --- 修正箇所 ---
        # st.secretsからos.environ.getに変更して、Renderの環境変数を直接読み込む
        creds_dict_str = os.environ.get("GOOGLE_CREDS_JSON")
        if not creds_dict_str:
            st.error("Google APIの認証情報（環境変数: GOOGLE_CREDS_JSON）が設定されていません。")
